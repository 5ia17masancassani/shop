...
public class CartService {
	...
	@PostMapping("/{id}/{size}/{token}")
	public ResponseEntity<Cart> addArticleToCart(@PathVariable("token")String tokenId, @PathVariable("id")int articleId, @PathVariable("size")String size) {
		...
		if (optCart.isPresent()) {
			...
			if (optArticle.isPresent()) {
				...
				optArticle.get().setSize(size);
				optArticle.get().setPrice(size);
	...
	@DeleteMapping("/all/{id}/{token}")
	public ResponseEntity<Cart> removeCartEntry(@PathVariable("token")String tokenId, @PathVariable("id")int articleId) {
		Optional<Cart> optCart = cartDAO.getCartByTokenId(tokenId);
		if (optCart.isPresent()) {
			List<Article> optArticle = articleDAO.getArticles();
			if (optArticle.size() != 0) {
				Cart cart = optCart.get();
				for (Article article: optArticle){
					cart.delAllArticle(article);
				}
